...
class MFQEv2Dataset(data.Dataset):
    ...
    def __init__(self, opts_dict, radius):
        ...
        # dataset paths
        self.gt_root = op.join('data/MFQEv2/', self.opts_dict['gt_path'])
        self.lq_root = op.join('data/MFQEv2/', self.opts_dict['lq_path'])
        # extract keys from meta_info.txt
        self.meta_info_path = op.join(self.gt_root, self.opts_dict['meta_info_fp'])  
        ...
        self.io_opts_dict['db_paths'] = [self.lq_root, self.gt_root]
        ...
        if radius == 0:
            ...
            nfs = 2 * radius + 1
            # self.neighbor_list = [i + (5 - nfs) // 2 for i in range(nfs)]  # 
            # self.neighbor_list = [i + (7 - nfs) // 2 for i in range(nfs)]
            self.neighbor_list = [i + (9 - nfs) // 2 for i in range(nfs)]

    def __getitem__(self, index):
        if self.file_client is None:
            self.file_client = FileClient(self.io_opts_dict.pop('type'), **self.io_opts_dict)
        # random reverse
        ...
        gt_size = self.opts_dict['gt_size']  # 128
        key = self.keys[index]
        ...
        # randomly crop
        img_gt, img_lqs = paired_random_crop(img_gt, img_lqs, gt_size, img_gt_path)
        # flip, rotate
        img_lqs.append(img_gt)  # gt joint augmentation with lq
        img_results = augment(img_lqs, self.opts_dict['use_flip'], self.opts_dict['use_rot']) 
        # to tensor
        img_results = totensor(img_results)
        img_lqs = torch.stack(img_results[0:-1], dim=0)
        img_gt = img_results[-1]

        return {'lq': img_lqs,  # (T [RGB] H W)
            'gt': img_gt,  # ([RGB] H W)
            }

    def __len__(self):
        return len(self.keys)  # 3357
...
class VideoTestMFQEv2Dataset(data.Dataset):
    ...
    def __init__(self, opts_dict, radius):
        ...
        self.data_info = {'lq_path': [], 'gt_path': [], 'gt_index': [], 'lq_indexes': [], 'h': [], 'w': [], 
            'index_vid': [], 'name_vid': [], }
        ...
        for idx_vid, gt_vid_path in enumerate(gt_path_list):
            ...
            lq_vid_path = op.join(self.lq_root, name_vid)
            for iter_frm in range(radius, nfs - radius):  
    ...
    def __getitem__(self, index):
        # get gt frame
        img = import_yuv(seq_path=self.data_info['gt_path'][index], h=self.data_info['h'][index], 
            w=self.data_info['w'][index], tot_frm=1, start_frm=self.data_info['gt_index'][index], only_y=True)
        img_gt = np.expand_dims(np.squeeze(img), 2).astype(np.float32) / 255.  # (H W 1)

        if self.data_info['h'][index] >= 1080:
            img_gt = img_gt[0:720, 0:1280, :]
        # if self.data_info['h'][index] >= 1080:
        #     img_gt = img_gt[0:1080, 0:1920, :]

        # get lq frames
        img_lqs = []
        for lq_index in self.data_info['lq_indexes'][index]:
            img = import_yuv(seq_path=self.data_info['lq_path'][index], h=self.data_info['h'][index],
            w=self.data_info['w'][index], tot_frm=1, start_frm=lq_index, only_y=True)
            img_lq = np.expand_dims(np.squeeze(img), 2).astype(np.float32) / 255.  # (H W 1)

            if self.data_info['h'][index] >= 1080:
                img_lq = img_lq[0:720, 0:1280, :]
            # if self.data_info['h'][index] >= 1080:
            #     img_lq = img_lq[0:1080, 0:1920, :]
        ...
        img_lqs = torch.stack(img_results[0:-1], dim=0)
        img_gt = img_results[-1]
        return {'lq': img_lqs,  # (T 1 H W)
            'gt': img_gt,  # (1 H W)
            'name_vid': self.data_info['name_vid'][index], 'index_vid': self.data_info['index_vid'][index],}
