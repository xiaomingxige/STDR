...
def import_yuv(seq_path, h, w, tot_frm, yuv_type='420p', start_frm=0, only_y=True, data_type = np.uint8):
    # setup params
    if yuv_type == '420p':
        hh, ww = h // 2, w // 2
    elif yuv_type == '444p':
    ...
    # init
    y_seq = np.zeros((tot_frm, h, w), dtype=data_type)
    if not only_y:
        u_seq = np.zeros((tot_frm, hh, ww), dtype=data_type)
        v_seq = np.zeros((tot_frm, hh, ww), dtype=data_type)

    # read data
    with open(seq_path, 'rb') as fp:
        for i in range(tot_frm):
            fp.seek(int(blk_size * (start_frm + i)), 0)  # skip frames
            y_frm = np.fromfile(fp, dtype=data_type, count=y_size).reshape(h, w)
            if only_y:
                y_seq[i, ...] = y_frm
            else:
                u_frm = np.fromfile(fp, dtype=data_type, count=u_size).reshape(hh, ww)
                v_frm = np.fromfile(fp, dtype=data_type, count=v_size).reshape(hh, ww)
...
class FileClient(object):
    ...
    _backends = {'disk': _HardDiskBackend, 'lmdb': _LmdbBackend,}
