...
def make_lmdb_from_imgs(img_dir,lmdb_path, img_path_list, keys, batch=5000, compress_level=1, multiprocessing_read=False, map_size=None):
    ...
    if map_size is None:
        ...
        img = cv2.imread(op.join(img_dir, img_path_list[0]), cv2.IMREAD_UNCHANGED)
    ...
    for idx, (path, key) in enumerate(zip(img_path_list, keys)):
        ...
        if multiprocessing_read:
            ...
            _, img_byte, img_shape = _read_img_worker(op.join(img_dir, path), key, compress_level)  # use _read function
            h, w, c = img_shape
        # write lmdb
...
def _read_y_from_yuv_worker(video_path, yuv_type, h, w, index_frame, key, compress_level):
    ...
    img = import_yuv(seq_path=video_path, yuv_type=yuv_type, h=h, w=w, tot_frm=1, start_frm=index_frame, only_y=True)
    img = np.squeeze(img)
    c = 1
    _, img_byte = cv2.imencode('.png', img, [cv2.IMWRITE_PNG_COMPRESSION, compress_level])
    return (key, img_byte, (h, w, c))
...
def make_y_lmdb_from_yuv(video_path_list, index_frame_list, key_list, lmdb_path, yuv_type='420p', h=None, w=None, 
    ...
    for iter_frm in range(num_img):
        pool.apply_async(_read_y_from_yuv_worker,args=(video_path_list[iter_frm], yuv_type, h, w, index_frame_list[iter_frm], key_list[iter_frm], 
                                                    compress_level), callback=_callback)
