...
parser.add_argument('--opt_path', type=str, default='option_R3_mfqev2_4G.yml', help='Path to option YAML file.')
...
def create_lmdb_for_mfqev2():
    # video info
    print('PID of the current process:{}'.format(os.getpid()))
    with open(yml_path, 'r') as fp:
        ...
        radius = fp['network']['radius'] # must be 3!!! otherwise, you should change dataset.py
    ...
    video_path_list = []
    index_frame_list = []
    for iter_vid in range(len(gt_video_list)):   # len(gt_video_list) = 108
        frms = frm_list[iter_vid]
        for iter_frm in range(len(frms)):
            # key_list.append('{:03d}/{:03d}/im2.png'.format(iter_vid+1, iter_frm+1))
            # key_list.append('{:03d}/{:03d}/im3.png'.format(iter_vid+1, iter_frm+1))
    ...
    make_y_lmdb_from_yuv(video_path_list=video_path_list, index_frame_list=index_frame_list, key_list=key_list, 
                    lmdb_path=lmdb_gt_path, multiprocessing_read=True,)
    print("> Finish.")
    ...
    make_y_lmdb_from_yuv(video_path_list=video_path_list,  index_frame_list=index_frame_list,  key_list=key_list, 
                        lmdb_path=lmdb_lq_path, multiprocessing_read=True,)
    print("> Finish.")
    # sym-link
